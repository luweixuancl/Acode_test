...
time_t TimeStamp::ToUnixTime() const {
    ...
    if (is_bjt) unixt -= 8 * 3600;
    return unixt;
}

TimeStamp TimeStamp::FromUnixTime(time_t t, bool as_bjt) {
    if (as_bjt) t += 8 * 3600;
    struct tm* gt = gmtime(&t);
    return TimeStamp(gt->tm_year + 1900, gt->tm_mon + 1, gt->tm_mday,
                     gt->tm_hour, gt->tm_min, gt->tm_sec, as_bjt);
}
...
std::string TimeStamp::ToFString(bool output_bjt) const {
    TimeStamp out = *this;
    if (output_bjt && !is_bjt) {
        time_t t = ToUnixTime() + 8 * 3600;
        out = FromUnixTime(t, true);
    }
    char buf[32];
...
bool IAGA2000Parser::ParseDataLine(const std::string& line, const std::map<std::string, int>& col_map,
    ...
    ts = TimeStamp(y, m, d, H, M, (int)S, false);
...
bool FFormatParser::Parse(const std::string& filename,
    ...
    while (std::getline(in, line)) {
        ...
        if (std::sscanf(line.c_str(), "%d/%d/%d %d:%d:%d %f", &y, &m, &d, &H, &M, &S, &f) == 7) {
            times.emplace_back(y, m, d, H, M, S, true);
...
bool ConfigParser::Parse(const std::string& filename, ReconConfig& config) {
    ...
    while (std::getline(in, line)) {
        ...
        if (section == "General") {
            ...
            if (key == "Count") {
                // 预留
            } else if (key.substr(0, 7) == "Station") {
    ...
    float total_w = 0;
    for (auto& s : config.input_stations) total_w += 1.0f / (s.distance_km * s.distance_km);
    for (auto& s : config.input_stations) {
        s.weight = (1.0f / (s.distance_km * s.distance_km)) / total_w;
    }
...
bool OutputWriter::WriteF(const std::string& filename,
    ...
    for (size_t i = 0; i < data.size(); ++i) {
        ...
        out << times[i].ToFString(output_bjt) << "  "
