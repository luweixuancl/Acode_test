...
- GPS owns `LocalClock`: PPS edges via `esp_timer`, ppm from `CLK_PPM_SPAN_SEC` ring average + EMA, LocalUtc extrapolation; NMEA residual cross-check (warn 50 ms / fail 100 ms). States ACQ/LCK/DEG/HLD/UNS. Holdover age uses `esp_timer`. `nowUtc()` serves NTP only from Locked/Degraded/Holdover. Acquire waits for `ppsStable_`.
