...
6. **网页配网**：开启 SoftAP（`NTP-Setup-XXXX` / 密码默认 **`NTP-`+MAC 后 4 位十六进制**，与串口打印的 MAC 对应；**仅 2.4 GHz**）。打开 `/setup` 时浏览器先弹出登录（用户名 **`admin`**，口令默认同 SoftAP）；通过后才显示设置页。`/` `/status` `/metrics` 只读开放。手机/电脑若连在 5 GHz 上可能扫不到该热点。
7. **网页状态**：STA 连上后访问 `http://<设备IP>/` 查看 NTP/GPS/PPS（JS 按 1 Hz 轮询 `/status`，无需整页刷新）；`/setup` 改 WiFi/策略（需上述登录）
...
4. **管理面**：`/` `/status` `/metrics` 只读开放；`/setup` `/save` `/scan` 用浏览器 Basic 登录（用户名 `admin`，口令默认 SoftAP `NTP-`+MAC 后 4 位）。
