...
2. **旋转编码器菜单**：扫描 WiFi、网页配网、静态 IP、DHCP、时区、重启
3. **手动静态 IP**：编码器逐字节编辑；保存时通过 **ARP 探测**检测局域网是否已有相同 IP
4. **编码器配网**：扫描附近 WiFi → 选择 SSID → 编码器输入密码 → 连接
5. **网页配网**：开启 SoftAP（`NTP-Setup-XXXX` / 密码 `12345678`），浏览器选择 WiFi 并输入密码
